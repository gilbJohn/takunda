...
function getFriendIdFromPair(userId: string, row: { user_id: string; friend_id: string }): string {
...
async function getPendingInvitesMock(userId: string): Promise<FriendInvite[]> {
  ...
  for (const inv of invites) {
    const sender = MOCK_USERS.find((u) => u.id === inv.sender_id) ?? (await getUser(inv.sender_id));
    if (sender) {
      result.push({
        id: inv.id,
...
async function getSentInvitesMock(userId: string): Promise<FriendInvite[]> {
  ...
  const result: FriendInvite[] = [];
  for (const inv of invites) {
    const recipient = MOCK_USERS.find((u) => u.id === inv.recipient_id) ?? (await getUser(inv.recipient_id));
    if (recipient) {
      result.push({
        id: inv.id,
        senderId: inv.sender_id,
...
async function sendFriendInviteMock(senderId: string, recipientId: string): Promise<FriendInvite | null> {
  if (senderId === recipientId) return null;
  const existing = mockFriendInvites.find(
    (i) => (i.sender_id === senderId && i.recipient_id === recipientId) ||
      (i.sender_id === recipientId && i.recipient_id === senderId)
  );
  if (existing) return null;
  const alreadyFriends = mockFriendships.some(
    (f) => (f.user_id === senderId && f.friend_id === recipientId) ||
  ...
  const recipient = MOCK_USERS.find((u) => u.id === recipientId) ?? (await getUser(recipientId));
  return { id, senderId, recipientId, createdAt: new Date().toISOString(), sender: sender ?? undefined, recipient: recipient ?? undefined };
}

async function sendFriendInviteSupabase(senderId: string, recipientId: string): Promise<FriendInvite | null> {
  ...
  return { id: data.id, senderId: data.sender_id, recipientId: data.recipient_id, createdAt: data.created_at, sender: sender ?? undefined };
}

export async function sendFriendInvite(senderId: string, recipientId: string): Promise<FriendInvite | null> {
...
async function acceptFriendInviteMock(inviteId: string, userId: string): Promise<boolean> {
  const inv = mockFriendInvites.find((i) => i.id === inviteId && i.recipient_id === userId);
  if (!inv) return false;
  mockFriendInvites = mockFriendInvites.filter((i) => i.id !== inviteId);
  const [u1, u2] = inv.sender_id < inv.recipient_id ? [inv.sender_id, inv.recipient_id] : [inv.recipient_id, inv.sender_id];
  mockFriendships.push({ user_id: u1, friend_id: u2 });
  return true;
}

async function acceptFriendInviteSupabase(inviteId: string, userId: string): Promise<boolean> {
  if (!supabase) return false;
  const { data: inv } = await supabase.from("friend_invites").select("sender_id, recipient_id").eq("id", inviteId).eq("recipient_id", userId).single();
  if (!inv) return false;
  const { error: delErr } = await supabase.from("friend_invites").delete().eq("id", inviteId);
  if (delErr) throw new Error(delErr.message);
  const [u1, u2] = inv.sender_id < inv.recipient_id ? [inv.sender_id, inv.recipient_id] : [inv.recipient_id, inv.sender_id];
  const { error: insErr } = await supabase.from("friendships").insert({ user_id: u1, friend_id: u2 });
  if (insErr) throw new Error(insErr.message);
  return true;
}

export async function acceptFriendInvite(inviteId: string, userId: string): Promise<boolean> {
  if (API_CONFIG.useSupabase) return acceptFriendInviteSupabase(inviteId, userId);
  return acceptFriendInviteMock(inviteId, userId);
}

async function declineFriendInviteMock(inviteId: string, userId: string): Promise<boolean> {
  const inv = mockFriendInvites.find((i) => i.id === inviteId && i.recipient_id === userId);
  if (!inv) return false;
  mockFriendInvites = mockFriendInvites.filter((i) => i.id !== inviteId);
  return true;
...
async function declineFriendInviteSupabase(inviteId: string, userId: string): Promise<boolean> {
  if (!supabase) return false;
  const { error } = await supabase.from("friend_invites").delete().eq("id", inviteId).eq("recipient_id", userId);
  if (error) throw new Error(error.message);
  return true;
}

export async function declineFriendInvite(inviteId: string, userId: string): Promise<boolean> {
...
async function getFriendSuggestionsSupabase(userId: string): Promise<User[]> {
  if (!supabase) return [];
  const { data: enrollments } = await supabase.from("user_classes").select("class_id").eq("user_id", userId);
  const myClassIds = (enrollments ?? []).map((e) => e.class_id);
  if (myClassIds.length === 0) return [];
  const { data: profiles } = await supabase.from("profiles").select("id, name, email, avatar, phone, school, major, onboarding_completed");
  ...
  const usersWithClasses = allUsers.map((u) => ({ ...u, classIds: classIdsByUser[u.id] ?? [] }));

  const { data: friendRows } = await supabase.from("friendships").select("user_id, friend_id").or(`user_id.eq.${userId},friend_id.eq.${userId}`);
  const friendIds = new Set((friendRows ?? []).map((r) => getFriendIdFromPair(userId, r)));

  const { data: sentInvites } = await supabase.from("friend_invites").select("recipient_id").eq("sender_id", userId);
  const { data: recvInvites } = await supabase.from("friend_invites").select("sender_id").eq("recipient_id", userId);
  const invitedIds = new Set([
